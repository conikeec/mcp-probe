        ...
        match config {
            ...
            TransportConfig::Stdio(_) => {
                Err(crate::error::ConfigError::InvalidValue {
                    ...
                    reason: "stdio transport support not compiled in (enable 'stdio' feature)".to_string(),
                }.into())
            }
            ...
            TransportConfig::HttpSse(_) => {
                Err(crate::error::ConfigError::InvalidValue {
                    ...
                    reason: "http-sse transport support not compiled in (enable 'http-sse' feature)".to_string(),
                }.into())
            }
            ...
            TransportConfig::HttpStream(_) => {
                Err(crate::error::ConfigError::InvalidValue {
                    parameter: "transport_type".to_string(),
                    value: "http-stream".to_string(),
                    reason: "http-stream transport support not compiled in (enable 'http-stream' feature)".to_string(),
                }.into())
            }
        }
    }
