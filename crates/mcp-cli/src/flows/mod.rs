...
use mcp_core::{
    error::McpResult,
    messages::{Implementation, JsonRpcMessage, InitializeRequest, InitializeResponse, ProtocolVersion},
...
impl FlowStepEnum {
    ...
    pub async fn execute(&self, context: &mut FlowContext, transport: &mut Box<dyn Transport>) -> McpResult<()> {
        match self {
...
pub trait FlowHandler: std::fmt::Debug + Send + Sync {
    /// Execute this flow step
    async fn execute(&self, context: &mut FlowContext, transport: &mut Box<dyn Transport>) -> McpResult<()>;
...
impl Connect {
    ...
    pub fn with_default_timeout() -> FlowStepEnum {
        FlowStepEnum::Connect(ConnectStep { timeout: Duration::from_secs(30) })
    }
}
...
impl FlowHandler for ConnectStep {
    async fn execute(&self, context: &mut FlowContext, transport: &mut Box<dyn Transport>) -> McpResult<()> {
        ...
        timeout(self.timeout, transport.connect()).await
            .map_err(|_| mcp_core::error::McpError::Transport(
        ...
        context.timing.step_timings.insert("connect".to_string(), step_start.elapsed());
        
        tracing::info!("Transport connected in {:?}", step_start.elapsed());
        Ok(())
    }
    
    fn step_name(&self) -> &'static str { "connect" }
...
impl FlowHandler for InitializeStep {
    async fn execute(&self, context: &mut FlowContext, transport: &mut Box<dyn Transport>) -> McpResult<()> {
        ...
        transport.send_request(json_request, Some(Duration::from_secs(30))).await?;
        
        context.timing.step_timings.insert("initialize".to_string(), step_start.elapsed());
        tracing::info!("Initialize request sent in {:?}", step_start.elapsed());
        Ok(())
    }
    
    fn step_name(&self) -> &'static str { "initialize" }
}
...
impl FlowHandler for WaitForResponseStep {
    async fn execute(&self, context: &mut FlowContext, transport: &mut Box<dyn Transport>) -> McpResult<()> {
        ...
        let message = timeout(self.timeout, transport.receive_message(Some(self.timeout))).await
            .map_err(|_| mcp_core::error::McpError::Transport(
                mcp_core::error::TransportError::ConnectionFailed {
                    transport_type: "generic".to_string(),
        ...
        match message {
            JsonRpcMessage::Response(response) => {
                if let Some(result) = response.result {
                    ...
                    context.timing.step_timings.insert("wait_response".to_string(), step_start.elapsed());
    ...
    fn step_name(&self) -> &'static str { "wait_response" }
    fn timeout(&self) -> Option<Duration> { Some(self.timeout) }
}
...
impl FlowHandler for ProcessCapabilitiesStep {
    async fn execute(&self, context: &mut FlowContext, _transport: &mut Box<dyn Transport>) -> McpResult<()> {
        let step_start = Instant::now();
        ...
        if let Some(response_value) = context.metadata.get("init_response") {
            ...
            if let Some(tools) = capabilities_value.get("tools") {
                if let Some(tools_obj) = tools.as_object() {
                    context.capabilities.tools.list_allowed = tools_obj.get("list_allowed")
            ...
            context.timing.step_timings.insert("process_capabilities".to_string(), step_start.elapsed());
            tracing::info!("Processed {} capabilities in {:?}", capability_count, step_start.elapsed());
            Ok(())
        } else {
            Err(mcp_core::error::McpError::Protocol(
                mcp_core::error::ProtocolError::InvalidResponse {
    ...
    fn step_name(&self) -> &'static str { "process_capabilities" }
}
...
impl FlowHandler for SendNotificationStep {
    async fn execute(&self, context: &mut FlowContext, transport: &mut Box<dyn Transport>) -> McpResult<()> {
        let step_start = Instant::now();
        context.state = NegotiationState::Finalizing { notifications_pending: 1 };
        ...
        context.timing.step_timings.insert("send_notification".to_string(), step_start.elapsed());
        tracing::info!("Sent {} notification in {:?}", self.notification_type, step_start.elapsed());
        Ok(())
    }
    
    fn step_name(&self) -> &'static str { "send_notification" }
}
...
impl FlowHandler for TransitionToStep {
    async fn execute(&self, context: &mut FlowContext, _transport: &mut Box<dyn Transport>) -> McpResult<()> {
        ...
        context.timing.step_timings.insert("transition_ready".to_string(), step_start.elapsed());
        tracing::info!("Transitioned to ready state in {:?}", step_start.elapsed());
        tracing::info!("Total negotiation completed in {:?}", context.timing.total_time);
        Ok(())
    }
    
    fn step_name(&self) -> &'static str { "transition_ready" }
}
...
impl NegotiationFlow {
    ...
    pub async fn execute(&mut self, transport_config: TransportConfig) -> Result<&FlowContext> {
        let mut transport = mcp_core::transport::factory::TransportFactory::create(transport_config).await?;
        ...
        for (index, step) in self.steps.iter().enumerate() {
            ...
            loop {
                ...
                match step.execute(&mut self.context, &mut transport).await {
                    Ok(()) => {
                        tracing::debug!("Step {} '{}' completed successfully", index, step.step_name());
                        break;
                    }
                    Err(error) => {
                        tracing::warn!("Step {} '{}' failed (attempt {}): {}", 
                                     index, step.step_name(), attempts, error);
                        
                        if attempts >= max_attempts || !step.is_retryable() {
                            self.context.state = NegotiationState::Failed {
                                reason: format!("Step '{}' failed after {} attempts: {}", 
                                              step.step_name(), attempts, error)
                            };
                        ...
                        let delay = Duration::from_millis(
                            (self.config.retry_policy.initial_delay.as_millis() as f64
                                * self.config.retry_policy.backoff_multiplier.powi((attempts - 1) as i32)) as u64
                        );
